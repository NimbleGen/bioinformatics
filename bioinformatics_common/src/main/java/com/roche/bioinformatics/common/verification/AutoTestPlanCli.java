...
public class AutoTestPlanCli {
	...
	public final static CommandLineOption USAGE_OPTION = new CommandLineOption("Print Usage", "usage", 'h',
			"Print Usage.", false, true);
	public final static CommandLineOption TEST_PLAN_DIRECTORY_OPTION = new CommandLineOption("Test Plan Directory",
			"testPlanDir", null, "Path to the Test Plan Directory", true, false);
	public final static CommandLineOption TEST_PLAN_EXECUTION_RESULTS_DIRECTORY_OPTION = new CommandLineOption(
			"Test Plan Execution Directory", "runDir", null,
			"Base directory for placing results file from running the test plan", true, false);
	public final static CommandLineOption APPLICATION_JAR_FILE_OPTION = new CommandLineOption("Application Jar File",
			"application", null, "The path to the application jar file to be tested.", true, false);
	public final static CommandLineOption JVM_BIN_PATH_OPTION = new CommandLineOption("JVM Bin Path", "jvm", null,
			"The path to the jvm bin to use for executing the tests.  If not provided, the application will use the default JVM.",
			false, false);
	public final static CommandLineOption OUTPUT_FILE_OPTION = new CommandLineOption("Output File", "output", null,
			"The output file to write the test plan or report.", true, false);
	public final static CommandLineOption ZIP_RESULTS_OPTION = new CommandLineOption("Zip Results", "zipResults", null,
			"Zip the results files and place them in the output directory then delete the results directory.", false,
			true);
	public final static CommandLineOption STARTING_FOLDER_OPTION = new CommandLineOption("Starting Folder",
			"startFolder", null, "Starts at the provided folder name if it exists and skips all previous tests.", false,
			false);
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (logFile != null) {
				CliStatusConsole.logError("You may find additional details regarding your error in the log file ["
						+ logFile.getAbsolutePath() + "].");
			} else {
				t.printStackTrace();
			}
			CliStatusConsole.logError(
					"If you are unable to fix this issue and believe the application is in error please contact technical support at \"http://www.nimblegen.com/contact\".");
			CliStatusConsole.logError("");
			CliStatusConsole.logError(underlineFrame);
			CliStatusConsole.logError("");
			System.exit(1);
		...
		if (showUsage) {
			if (activeCommand != null) {
				...
				if (noOptionsProvided) {
					...
					CliStatusConsole
							.logStatus("As an example, a Test Plan would be generated by executing the following:");
					CliStatusConsole.logStatus(
							APPLICATION_NAME + " " + TEST_PLAN_COMMAND_NAME + " <test plan specific arguments>");
					CliStatusConsole.logStatus("");
				} else {
					CliStatusConsole.logStatus("");
					CliStatusConsole.logStatus("___________________________________________________________________");
			...
			if (activeCommand != null) {
				if (activeCommand.getCommandName().equals(TEST_PLAN_COMMAND_NAME)) {
					runTestPlan(parsedCommandLine, commandLineSignature, APPLICATION_NAME,
							applicationVersionFromManifest, false);
				} else if (activeCommand.getCommandName().equals(TEST_PLAN_REPORT_COMMAND_NAME)) {
					runTestPlan(parsedCommandLine, commandLineSignature, APPLICATION_NAME,
							applicationVersionFromManifest, true);
				} else {
	...
	private static Commands getCommands() {
		Commands commands = new Commands(
				"Available Commands:" + StringUtil.NEWLINE + "Command" + StringUtil.TAB + StringUtil.TAB + "Description"
						+ StringUtil.NEWLINE + "------" + StringUtil.TAB + StringUtil.TAB + "-----------");
		commands.addCommand(
				new Command(TEST_PLAN_COMMAND_NAME, "Create a Test Plan based on the provided Test Plan directory.",
						getCommandLineOptionsGroupForTestPlan()));
		commands.addCommand(new Command(TEST_PLAN_REPORT_COMMAND_NAME,
				"Create a Test Plan Report (which will actually run the test plan) based on the provided Test Plan directory.",
				getCommandLineOptionsGroupForTestPlanReport()));
		return commands;
	}

	public static File getLogFile(File outputDirectory, String outputFilePrefix, String applicationName,
			String command) {
		if (outputFilePrefix == null) {
			outputFilePrefix = "";
		}
		String logFileName = outputFilePrefix;
		logFileName = logFileName + applicationName + "_" + command + "_"
				+ DateUtil.getCurrentDateINYYYY_MM_DD_HH_MM_SS() + ".log";
	...
	public static File getLogFile(File outputFile, String applicationName, String command) {
		String logFileName = applicationName + "_" + command + "_" + DateUtil.getCurrentDateINYYYY_MM_DD_HH_MM_SS()
				+ ".log";
		logFileName = logFileName.replaceAll(" ", "_");
		logFileName = logFileName.replaceAll("/", "_");
		logFileName = logFileName.replaceAll(":", "-");
	...
	public static void runTestPlan(ParsedCommandLine parsedCommandLine, String commandLineSignature,
			String applicationName, String applicationVersion, boolean generateReport) {
		long applicationStart = System.currentTimeMillis();
		CliStatusConsole.logStatus("The generation of the Test Plan has started at "
				+ DateUtil.convertTimeInMillisecondsToDate(applicationStart) + "(YYYY/MM/DD HH:MM:SS)."
				+ StringUtil.NEWLINE);
			...
			if (!outputFile.exists()) {
				try {
					...
					throw new IllegalStateException(
							"Could not create provided output file[" + outputFile.getAbsolutePath() + "].", e);
				}
			}
		}
		...
		if (!testPlanDirectory.isDirectory()) {
			throw new IllegalArgumentException(
					"The provided file for " + TEST_PLAN_DIRECTORY_OPTION.getLongFormOption() + " is not a directory.");
		}

		if (!testPlanDirectory.exists()) {
			throw new IllegalArgumentException(
					"The provided file for " + TEST_PLAN_DIRECTORY_OPTION.getLongFormOption() + " does not exist.");
		}
		...
		if (generateReport) {
			...
			if (parsedCommandLine.isOptionPresent(JVM_BIN_PATH_OPTION)) {
				...
				if (!jvmBinFile.exists()) {
					throw new IllegalArgumentException("The provided argument for "
							+ JVM_BIN_PATH_OPTION.getLongFormOption() + "[" + jvmBinPath + "] does not exist.");
				}
			}
			...
			if (!appToTest.exists()) {
				throw new IllegalArgumentException(
						"The provided argument for " + APPLICATION_JAR_FILE_OPTION.getLongFormOption() + "["
								+ appToTestString + "] does not exist.");
			}

			File testPlanExecutionDirectory = new File(
					parsedCommandLine.getOptionsValue(TEST_PLAN_EXECUTION_RESULTS_DIRECTORY_OPTION));
